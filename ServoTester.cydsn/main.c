...
void UpdateRGB(void)
{
	...
	uint16 sliderPosition = CapSense_GetCentroidPos(CapSense_LINEARSLIDER0__LS);
	...
	if(sliderPosition != NO_FINGER)
	{
    	...
    	if(sliderPosition != lastPosition)
    	{
            ...
            uint16 rc_pwm = (1000 + ((uint32)sliderPosition * 1000) / 255) / 10; 
            RC_OUT_WriteCompare(rc_pwm);
